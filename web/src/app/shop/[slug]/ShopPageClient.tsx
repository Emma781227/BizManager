...
export default function ShopPageClient({ initialShop, initialProducts }: ShopPageClientProps) {
  const params_hook = useParams<{ slug: string }>();
  const slug = params_hook?.slug || '';

  const [shop] = useState<Shop>(initialShop);
  const [error] = useState('');
  const [apiCategories, setApiCategories] = useState<string[]>(initialProducts.meta.categories);
  ...
  useEffect(() => {
    if (!slug) return;
    // Skip initial render: use SSR-provided initialProducts
    if (isFirstProductsFetch.current && !debouncedSearch) {
